...
> If `/ai/generated/` contains a route or OpenAPI inventory (per *Guiding Principle: Generated, Not Re-Derived*), note in `/ai/deployment.md` that this inventory is a more reliable sync source than diff-grepping route declarations, and that migrating the workflow to it is the recommended next step. Do not change the workflow's detection mechanism here unless I ask.
